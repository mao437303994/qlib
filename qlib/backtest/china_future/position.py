...
class PositionD:
    ...
    open_commission_rate: float  # 开仓费率
    open_commission_per_lot: float  # 开仓费用（按手）
    close_commission_rate: float  # 平仓费率
    close_commission_per_lot: float  # 平仓费用（按手）
    long_margin_rate: float  # 做多保证金率
    short_margin_rate: float  # 做空保证金率
...
class FuturePosition(BasePosition):
    ...
    def __init__(
        self,
        cash: float = 0,
        position_dict: Dict[str, Dict[str, PositionD]] = {},
    ) -> None:
